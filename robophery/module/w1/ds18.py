...
class Ds18Module(W1Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._type = kwargs.get('type', 10)#'DS18S20')
    ...
    def read_data(self):
        ...
        if self._addr in ['0', None]:
            ...
            if len(raw_data) == 0:
                ...
                read_time_delta = (read_time_stop - read_time_start) / len(raw_data)
                for raw_name, raw_value in raw_data.items():
                    data.append(('{0}-{1}'.format(self._name, raw_name), 'temperature', raw_value, read_time_delta))
        else:
            read_time_start = time.time()
            temperature = self._get_temperature(self._addr, self._type)
            read_time_stop = time.time()
            read_time_delta = read_time_stop - read_time_start
            data.append((self._name, 'temperature', temperature, read_time_delta))
        self._log_data(data)
        return data
